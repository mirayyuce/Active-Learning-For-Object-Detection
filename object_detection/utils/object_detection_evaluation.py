...
class ObjectDetectionEvaluator(DetectionEvaluator):
  ...
  def evaluate(self):
    ...
    #  num_gt_instances) = (self._evaluation.evaluate())
    """(per_class_ap, mean_ap, precisions_per_class, recalls_per_class, per_class_corloc, mean_corloc,
      num_gt_instances, scores) = (self._evaluation.evaluate())
    pascal_metrics = {
        self._metric_prefix +
    ...
    for idx in range(per_class_ap.size):
      if idx + self._label_id_offset in category_index:
        ...
        display_name_precision = (
            self._metric_prefix + 'precisions_per_class{}'.format(category_name))
        pascal_metrics[display_name_precision] = np.var(precisions_per_class[idx])


        display_name_recall = (
            self._metric_prefix + 'recalls_per_class{}'.format(category_name))
        pascal_metrics[display_name_recall] = np.var(recalls_per_class[idx])

        #print("scores[idx]",scores[idx])
        #print("len(scores[idx]) ",len(scores[idx]))
        if len(scores[idx]) == 0:
          result = np.nan

        else:
          #print("scores[idx][0]",scores[idx][0])
          indices = np.argwhere(scores[idx][0] > 0.1)
          #print(indices)
          if indices.size == 0:
            result = np.nan
          else:
            #result = np.var(list(scores[idx][0][np.concatenate(indices)]))
            result = np.std(list(scores[idx][0][np.concatenate(indices)]))
        #print("result",result)
        display_name_scores = (
            self._metric_prefix + 'scores{}'.format(category_name))
        pascal_metrics[display_name_scores] = result

        display_name_num_gt_instances = (
            self._metric_prefix + 'num_gt_instances{}'.format(category_name))
        pascal_metrics[display_name_num_gt_instances] = num_gt_instances[idx]
...
ObjectDetectionEvalMetrics = collections.namedtuple(
    'ObjectDetectionEvalMetrics', [
        ...
        'mean_corloc'#,"num_gt_ins_per_class","scores", 
    ])
...
class ObjectDetectionEvaluation(object):
  ...
  def evaluate(self):
    ...
    for class_index in range(self.num_class):
      ...
      num_gt_ins_per_class = self.num_gt_instances_per_class[class_index]
