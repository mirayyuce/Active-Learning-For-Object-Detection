...
tf.app.flags.DEFINE_string('data_dir', '', 'Location of root directory for the '
                           ...
                           '(images).')
tf.app.flags.DEFINE_string('images_list_path', '', 'Location of root directory for the unlabeled_set.txt')
...
def convert_kitti_to_tfrecords(data_dir, images_list_path, output_path, classes_to_use,
  ...
  Args:
    ...
    images_list_path: Location of unlabeled_set.txt.
  ...
  # Read all 6500 images
  unlabeled_images = get_images_list(images_list_path, "unlabeled_set.txt")

  for round_number in range(5):
    # Read 1300 images based on the active learning round
    round_images = unlabeled_images[round_number * 1300:(round_number + 1) * 1300]
    
    # Sample images with random uniform distribution from the set of 1300 images
    random_set = get_subset(round_images, budget, output_path, round_number)
    
    # Create the tfrecords
    create_tfrecords(random_set, output_path, image_dir, annotation_dir, classes_to_use, label_map_dict, "random", round_number)

    print("Random " + str(round_number) + " tfrecords have been written to " + output_path)


def get_images_list(images_list_path, set_name):
  """Read images lists.
  Args:
    images_list_path: Absolute path to init_set.txt or test_set.txt
    set_name: Dataset split name, unlabeled_set.txt
  """
  images_list = []
  with open(os.path.join(images_list_path, set_name), "r") as f:
...
def get_subset(round_images, budget, output_path, round_number):
  random_set_indices = np.random.choice(np.arange(len(round_images)), budget, replace = False)
    
  random_set = [round_images[index] for index in random_set_indices]
    
  with open(os.path.join(output_path,"random_selected_images_" + str(round_number) + ".txt"), "w") as f:
