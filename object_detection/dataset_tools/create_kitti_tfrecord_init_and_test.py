...
tf.app.flags.DEFINE_string('data_dir', '', 'Location of root directory for the '
                           ...
                           '(images).')
tf.app.flags.DEFINE_string('images_list_path', '', 'Location of root directory for the init_set.txt and test_set.txt')
tf.app.flags.DEFINE_string('output_path', '', 'Path to which TFRecord files'
                           'will be written. The TFRecord with the training set'
                           'will be located at: <output_path>/init_set.tfrecord.'
...
def convert_kitti_to_tfrecords(data_dir, images_list_path, output_path, classes_to_use,
                               label_map_path):
  """Prepare required information for tfrecords and call the create_tfrecords().
  Args:
    ...
    images_list_path: Absolute path to init_set.txt and test_set.txt.
    output_path: The path to which TFRecord files will be written. The TFRecord
      with the training set will be located at: <output_path>/init_set.tfrecord
  ...
  init_images = get_images_list(images_list_path, "init_set.txt")
  
  # Create the tfrecords
  create_tfrecords(init_images, output_path, image_dir, annotation_dir, classes_to_use, label_map_dict, "init_set")

  print("Init_set tfrecords have been written to " + output_path)
  
  test_images = get_images_list(images_list_path, "test_set.txt")

  # Create the tfrecords
  create_tfrecords(test_images, output_path, image_dir, annotation_dir, classes_to_use, label_map_dict, "test_set")

  print("Test_set tfrecords have been written to " + output_path)


def create_tfrecords(images_list, output_path, image_dir, annotation_dir, classes_to_use, label_map_dict, set_name):
  ...
  tf_writer = tf.python_io.TFRecordWriter('%s/%s.tfrecord'%
...
def get_images_list(images_list_path, set_name):
  ...
  Args:
    ...
    set_name: Dataset split name, init_set.txt or test_set.txt
  """
  images_list = []
