...
tf.app.flags.DEFINE_string('data_dir', '/home/aev21/data/KITTI/raw_data', 'Location of root directory for the '
                           ...
                           '(images).')
tf.app.flags.DEFINE_string('al_data_dir', '/home/aev21/data/KITTI/images_for_al.txt', 'Location of root directory for the '
                           'data. Folder structure is assumed to be:'
                           '<data_dir>/training/label_2 (annotations) and'
                           '<data_dir>/data_object_image_2/training/image_2'
                           '(images).')
tf.app.flags.DEFINE_string('output_path', '/home/aev21/data/KITTI/random_set/', 'Path to which TFRecord files'
...
tf.app.flags.DEFINE_integer('random_set_size', '300', 'Number of images to'
                            'be used as a validation set.')
tf.app.flags.DEFINE_integer('random_set_id', '1', 'Number of images to'
                            'be used as a validation set.')
...
def convert_kitti_to_tfrecords(data_dir, al_data_dir, output_path, classes_to_use,
                               label_map_path, random_set_size, random_set_id):
  ...
  image_dir = os.path.join(data_dir,
                           ...
                           'training',
                           'image_2')
  output_path_random_size = output_path + str(random_set_size) + "/"
  if not os.path.isdir(output_path_random_size):
    os.mkdir(output_path_random_size)

  random_set = get_subset(al_data_dir, random_set_size)

  #train_set = []

  #with open(al_data_dir[:-17]+"images_for_train.txt", "r") as f:
  #  train_images = f.readlines()

  #  for idx, tr_img in enumerate(train_images):
  #    train_set.append(tr_img[:10])

  #f.close()

  #merged_set = train_set + random_set
  #np.random.shuffle(merged_set)

  #print(len(merged_set))

  #with open(al_data_dir[:-17]+"images_for_train.txt", 'w') as f_merged:
  #  for img in merged_set:
  #    f_merged.write("%s\n" % img[:10])
  #f_merged.close()

  random_writer = tf.python_io.TFRecordWriter('%s_%d_random.tfrecord'%
                                           (output_path_random_size, random_set_id))
...
def read_annotation_file(filename):
  ...
  """def get_subset(path_to_images, random_set_size):
    random_set_indices = []
    ...
    with open(path_to_images, "r") as f:
      ...
      #random_set_indices = np.random.choice(np.arange(len(al_images)), random_set_size, replace = True)
      #update images_for_al.txt
  ...
  """
def get_subset(path_to_images, random_set_size):
  path_to_images_al = path_to_images
  random_set_indices = []
  al_images = []
  with open(path_to_images, "r") as f:
    al_images = f.readlines()
  ...
  with open(path_to_images_al, 'w') as f:
...
def main(_):
  convert_kitti_to_tfrecords(
      ...
      random_set_id=FLAGS.random_set_id)
