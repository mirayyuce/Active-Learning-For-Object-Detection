...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def _postprocess_box_classifier(self,
    ...
    #print(class_predictions_with_background_batch.shape)
  ...
  def _batch_decode_boxes(self, box_encodings, anchor_boxes):
    ...
    combined_shape = shape_utils.combined_static_and_dynamic_shape(
        box_encodings)
    #print(combined_shape) [50, 300, 3, 4]
    #print(box_encodings.shape)(50, 300, 3, 4)

    num_classes = combined_shape[2]
    #print(anchor_boxes)(50, 300, 3, 4)

    #print(anchor_boxes.shape)(1, 300, 4)

    
    tiled_anchor_boxes = tf.tile(
        tf.expand_dims(anchor_boxes, 2), [1, 1, num_classes, 1])

    #print(tiled_anchor_boxes)Tensor("SecondStagePostprocessor/Tile:0", shape=(1, 300, 3, 4), dtype=float32)

    #print(tiled_anchor_boxes.shape)

    tiled_anchors_boxlist = box_list.BoxList(
        tf.reshape(tiled_anchor_boxes, [-1, 4]))
    #print(tiled_anchors_boxlist) #<object_detection.core.box_list.BoxList object at 0x7fc2a8f96240>

    #print(tiled_anchors_boxlist.shape)

    decoded_boxes = self._box_coder.decode(
        tf.reshape(box_encodings, [-1, self._box_coder.code_size]),
        tiled_anchors_boxlist)
    #print("HEREEE")
    #print([-1, self._box_coder.code_size])
    ##print(decoded_boxes.shape)
    #print(tf.reshape(decoded_boxes.get(),
